...
public class Service {
  ...
  public Response modifica(@FormParam("usuario") User usuario) throws Exception {
    Connection conexion = pool.getConnection();

    if (usuario.email == null || usuario.email.equals(""))
      return Response.status(400).entity(j.toJson(new Error("Ingresar el email"))).build();

    if (usuario.nombre == null || usuario.nombre.equals(""))
      return Response.status(400).entity(j.toJson(new Error("Ingresar el nombre"))).build();

    if (usuario.apellido_paterno == null || usuario.apellido_paterno.equals(""))
      return Response.status(400).entity(j.toJson(new Error("Ingresar el apellido paterno"))).build();

    if (usuario.fecha_nacimiento == null || usuario.fecha_nacimiento.equals(""))
      return Response.status(400).entity(j.toJson(new Error("Ingresar la fecha de nacimiento"))).build();
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          return Response.status(400).entity(j.toJson(new Error("No hay registrado ningun usuario con ID " + id_usuario))).build();
        } finally {
          rs.close();
        }
      } finally {
  ...
  @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
  @Produces(MediaType.APPLICATION_JSON)
  public Response alta(@FormParam("usuario") User usuario) throws Exception {
    Connection conexion = pool.getConnection();
  ...
  public Response borra(@FormParam("id_usuario") int id_usuario) throws Exception {
    ...
    try {
      ...
      try {
        ...
        try {
          if (!rs.next())
            return Response.status(400).entity(j.toJson(new Error("No hay ningun usuario con ID " + id_usuario))).build();
        } finally {
          rs.close();
        }
      } finally {
